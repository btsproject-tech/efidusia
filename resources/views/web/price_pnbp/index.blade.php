@if (isset($akses->price_pnbp))
    @if ($akses->price_pnbp->view == 1)
        <input type="hidden" id="update" value="{{ $akses->price_pnbp->update }}">
        <input type="hidden" id="delete" value="{{ $akses->price_pnbp->delete }}">
        ...
        <div class="row">
            <div class="col-12">
                ...
                class="glyphicon glyphicon-screenshot"></span> Export</button></div> --}}
                                    @if ($akses->price_pnbp->insert == 1)
                                        <div class="btn-group dropdown-btn-group pull-right"><button type="button"
                                                class="btn btn-success" onclick="PricePnbp.add(this, event)"><i class="mdi mdi-plus me-1"></i>Tambah
                                                Data</button>
                                        </div>
                                    @endif
                                </div>

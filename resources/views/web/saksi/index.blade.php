@if (isset($akses->saksi))
    @if ($akses->saksi->view == 1)
        <input type="hidden" id="update" value="{{ $akses->saksi->update }}">
        <input type="hidden" id="delete" value="{{ $akses->saksi->delete }}">
        ...
        <div class="row">
            <div class="col-12">
                ...
                class="glyphicon glyphicon-screenshot"></span> Export</button></div> --}}
                                    @if ($akses->saksi->insert == 1)
                                        <div class="btn-group dropdown-btn-group pull-right"><button type="button"
                                                class="btn btn-success" onclick="Saksi.add(this, event)"><i class="mdi mdi-plus me-1"></i>Tambah
                                                Data</button>
                                        </div>
                                    @endif
                                </div>

@if (isset($akses->company))
    @if ($akses->company->view == 1)
        <input type="hidden" id="update" value="{{ $akses->company->update }}">
        <input type="hidden" id="delete" value="{{ $akses->company->delete }}">
        ...
        <div class="row">
            <div class="col-12">
                ...
                class="glyphicon glyphicon-screenshot"></span> Export</button></div> --}}
                                    @if ($akses->company->insert == 1)
                                        <div class="btn-group dropdown-btn-group pull-right"><button type="button"
                                                class="btn btn-success" onclick="Company.add(this, event)"><i class="mdi mdi-plus me-1"></i>Tambah
                                                Data</button>
                                        </div>
                                    @endif
                                </div>

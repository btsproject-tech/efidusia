...
class ActivityUser extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        $datadb = DB::table('actors as a')
            ...
            ->join('users as u', 'a.users', '=', 'u.id')
            ->join('karyawan as k', 'u.nik', '=', 'k.nik')
            ->whereNull('a.deleted')
            ->orderBy('a.id', 'desc');

        if (isset($data['tgl_awal'])) {
            if ($data['tgl_awal'] != '') {
                $datadb->where('a.created_at', '>=', $data['tgl_awal'])
                    ->where('a.created_at', '<=', $data['tgl_akhir']);
            }
        }
        ...
        if (isset($_POST)) {
            ...
            if (isset($_POST['order'][0]['column'])) {
                $datadb->orderBy('a.id', $_POST['order'][0]['dir']);
            }
    ...
    {
        $data = $request->all();
        return view('web.activity_user.modal.detailActivity', $data);
    }
